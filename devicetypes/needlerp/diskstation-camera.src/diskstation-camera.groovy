...
metadata {
    ...
    tiles {
        ...
        multiAttributeTile(name: "videoPlayer", type: "videoPlayer", width: 4, height: 2) {
...
*/          
        ...
        valueTile("vidTime", "device.vidTime", width:2, height: 1, inactiveLabel:true) {
            state "val", label:'${currentValue}', action: "getplayStatus", icon: "t", defaultstate: true
        }
...
def start() {
    ...
    if (device.currentState("playStatus")?.value == "video") {
        if (device.currentState("vidInfo") != null) {
            ...
            /*if (!event)
            	log.trace "video event" event
            else
                log.trace "video event null"*/
            log.trace "sending video event"
            sendEvent(event)
        } else {
            //log.trace "haven't yet retrieved the video id"
            getVideoDetails()
        }
    } else {
        ...
        /*if (!event)
        	log.trace "live stream event" event
        else 
            log.trace "live stream event null"*/
        log.trace "sending live event"
        sendEvent(event)
    }
...
def getInHomeURL() {
    ...
    def eventId = device.currentState("vidInfo")?.value
    if (device.currentState("playStatus")?.value == "video") {
        if (device.currentState("vidInfo").value.isInteger()) {
            ...
            log.trace "haven't yet retrieved the video id (homeURL)"
            //getVideoDetails() //This will cause recursion since it should be returning a value
            [InHomeURL: InHomeURL]
        }
    } else {
...
def getOutHomeURL() {
    ...
    if (device.currentState("playStatus")?.value == "video") {
        if (device.currentState("vidInfo").value.isInteger()) {
            ...
            log.trace "haven't yet retrieved the video id (remoteURL)"
            //getVideoDetails() //This will cause recursion since it should be returning a value
            [OutHomeURL: OutHomeURL]
        }
    } else {
...
def seteventTime(eventTime) {
    log.trace "child eventTime: " + eventTime
    if (eventTime == "MJPEG") {
...
def getVideoDetails() {
    ...
    log.trace "getVideoDetails live hubAction: " + hubAction
...
def getplayStatus() {
    ...
    sendEvent(name:"vidTime", value:"getting data...", displayed:false)
    def hubAction = queueDiskstationCommand_Child("SYNO.SurveillanceStation.Event", "List", "cameraIds=${cameraId}&orderMethod=1&limit=1", 5)
    log.trace "getVideoDetails Video hubaAction: " + hubAction
    //		updateHandler()
    return hubAction
...
def initChild(Map capabilities)
{
    ...
    if (device.currentState("playStatus")?.value == "Live") { //Needlerp
        ...
        //sendEvent(name:"startLive", value:"Live", isStateChange:true)
    } else {
    	//sendEvent(name: "playStatus", value: "Video")
        //sendEvent(name:"startLive", value:"Video", isStateChange:true)
    }
}
...
def queueDiskstationCommand_Child(String api, String command, String params, int version) {
    ...
    return parent.createHubAction(commandData)
    /*log.trace "queueDiskstationCommand_Child hubAction: "+ hubAction
    if (!hubAction)
    	log.trace "hubAction: " + hubAction
    else
        log.trace "hubAction null"
    return hubAction*/
}

def getStreamURL_Child(String api, String command, String params, int version, String location) {
	log.trace "creating commanddata and hubstream"
    def commandData = parent.createCommandData(api, command, params, version)
    return parent.createStreamURL(commandData, location)
    /*log.trace "queueDiskstationCommand_Child hubStream: "+ hubStream
    if (!hubStream)
    	log.trace "hubStream:" hubStream
    else
        log.trace "hubStream is null"
    hubStream*/ // return keyword is optional in groovy
}
